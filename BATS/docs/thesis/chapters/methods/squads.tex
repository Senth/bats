...
Now you know how it works, but which units will be used for the drop, as drops should not just use random units and hope for the best? It uses \nameref{sec:unit_composition}s for this—section \ref{sec:unit_composition} describes the unit composition and the drop's unit compositions can be found in listing \ref{lst:unit_comp_drop}. 

%: Write which drops are available in the current version of BATS
\marginpar{Write which drops are available in BATS}
